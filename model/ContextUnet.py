...
class ResidualBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # Time and context embedding projections
        self.time_emb_proj = nn.Sequential(
            nn.SiLU(), torch.nn.Linear(temb_channels, out_channels))

        self.cont_emb_proj = nn.Sequential(
            nn.SiLU(), torch.nn.Linear(cont_channels, out_channels))
    ...
    def forward(self, x, temb, cemb):
        ...
        cemb = self.cont_emb_proj(self.nonlinearity(cemb))
        x += temb[:, :, None, None]
        x += cemb[:, :, None, None]  # Incorporating context embedding
...
class UNet(nn.Module):
    ...
    def __init__(self,
        ...
        # EmbedFC: one layer feed-forward neural network for embedding input
        ...
        # Context embedding
        # context_input_dim = hidden_dims[0]
        context_input_dim = n_cfeat
        context_embed_dim = context_input_dim * 4

        self.context_embedding = nn.Sequential(
            nn.Linear(context_input_dim, context_embed_dim), nn.SiLU(),
        ...
        for idx, hidden_dim in enumerate(hidden_dims[1:]):
            ...
            down_blocks.append(
                nn.ModuleList([
                    ResidualBlock(in_dim, in_dim, time_embed_dim, context_embed_dim),
                    ResidualBlock(in_dim, in_dim, time_embed_dim, context_embed_dim),
        ...
        self.mid_block1 = ResidualBlock(mid_dim, mid_dim, time_embed_dim, context_embed_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = ResidualBlock(mid_dim, mid_dim, time_embed_dim, context_embed_dim)
        ...
        for idx, hidden_dim in enumerate(list(reversed(hidden_dims[:-1]))):
            ...
            up_blocks.append(
                nn.ModuleList([
                    ResidualBlock(in_dim + hidden_dim, in_dim, time_embed_dim, context_embed_dim),
                    ResidualBlock(in_dim + hidden_dim, in_dim, time_embed_dim, context_embed_dim),
                    get_attn_layer(in_dim, is_last, use_linear_attn),
        ...
        self.out_block = ResidualBlock(hidden_dims[0] * 2, hidden_dims[0],
                                       time_embed_dim, context_embed_dim)
        self.conv_out = nn.Conv2d(hidden_dims[0], out_channels=3, kernel_size=1)

    def forward(self, sample, timesteps, context=None):
        ...
        # context embedding
        if context is None: # mask out context if context_mask == 1
            context = torch.zeros(sample.shape[0], self.n_cfeat).to(sample)
        c_emb = self.context_embedding(context.float())
        ...
        x = self.out_block(torch.cat((x, r), dim=1), t_emb, c_emb)
        out = self.conv_out(x)
        return {"sample": out}